...
function SplashCursor({
  ...
  const pointers = useRef([new PointerPrototype()]).current
  let dye = null
  let velocity = null
  let divergence = null
  let curl = null
  ...
  useEffect(() => {
    ...
    const copyProgram = new Program(gl, vertexShader, compileShader(gl, gl.FRAGMENT_SHADER, copyShader))
    const clearProgram = new Program(gl, vertexShader, compileShader(gl, gl.FRAGMENT_SHADER, clearShader))
    const splatProgram = new Program(gl, vertexShader, compileShader(gl, gl.FRAGMENT_SHADER, splatShader))
    const advectionProgram = new Program(gl, vertexShader, compileShader(gl, gl.FRAGMENT_SHADER, advectionShader))
    const divergenceProgram = new Program(gl, vertexShader, compileShader(gl, gl.FRAGMENT_SHADER, divergenceShader))
    const curlProgram = new Program(gl, vertexShader, compileShader(gl, gl.FRAGMENT_SHADER, curlShader))
    const vorticityProgram = new Program(gl, vertexShader, compileShader(gl, gl.FRAGMENT_SHADER, vorticityShader))
    const pressureProgram = new Program(gl, vertexShader, compileShader(gl, gl.FRAGMENT_SHADER, pressureShader))
    const gradienSubtractProgram = new Program(
      gl,
      vertexShader,
      compileShader(gl, gl.FRAGMENT_SHADER, gradientSubtractShader),
    )
    const displayMaterial = new Material(vertexShader, displayShaderSource)
    ...
    function resizeDoubleFBO(gl, target, w, h, internalFormat, format, type, param) {
    ...
    function render(target) {
      ...
      gl.enable(gl.BLEND)
      drawDisplay(gl, target)
    }

    function drawDisplay(gl, target) {
    ...
    function splatPointer(pointer) {
      ...
      splat(gl, pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color)
    }
    ...
    function clickSplat(pointer) {
      ...
      splat(gl, pointer.texcoordX, pointer.texcoordY, dx, dy, color)
    }
    ...
    function blit(gl, target, clear = false) {
      if (target == null) {
